...
function ShapeEditor() {
  ...
  return (
    <div className="h-full max-h-screen overflow-y-auto flex flex-col gap-4 p-4">
      ...
      {shapes.map((shape) => {
        const update = (changes: Partial<Shape>) => dispatch(actions.shapes.updateOne({ id: shape.id, changes }));
        return (
          <Accordion key={shape.id} className="w-full border rounded-md" collapsible type="single">
            <AccordionItem value="shape1">
              ...
              <AccordionContent>
                <div className="grid gap-2 p-4">
                  ...
                  <Button variant="ghost" className="mt-3" onClick={() => dispatch(actions.shapes.removeOne(shape.id))}>
                    Delete
                  </Button>
                </div>
              </AccordionContent>

...
function ShapeThumbnail({ id }: { id: string }) {
  const shape = useAppSelector((state) => selectors.shapes.selectById(state, id));
  if (shape === undefined) {
    return null;
  }
  return (
    <svg className="w-full h-full" viewBox="0 0 1000 1000" xmlns="http://www.w3.org/2000/svg">
      <circle cx={500} cy={500} r={200} strokeWidth={30} fill={shape.fill} stroke={shape.stroke} />
    </svg>
  );
}
...
function KeyframeEditor() {
  ...
  const update = (changes: Partial<KeyFrame>) => dispatch(actions.keyframes.updateOne({ id: keyframe.id, changes }));

  return (
    <div className="flex gap-12 justify-between items-start p-6">
      <div className="flex flex-col gap-4 items-start mb-6">
        <h2 className="text-xl font-bold">Keyframe #{keyframe.id.replace('keyframe', '')}</h2>

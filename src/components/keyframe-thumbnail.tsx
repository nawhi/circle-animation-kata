...
export function KeyframeThumbnail({ id }: { id: string }) {
  const kf = useAppSelector((state) => selectors.keyframes.selectById(state, id));
  const entries = kf?.entries ?? [];
  const shapes = useAppSelector(selectors.shapes.selectEntities);

  return (
    <svg className="w-full h-full" viewBox={`0 0 ${CANVAS_WIDTH} ${CANVAS_HEIGHT}`} xmlns="http://www.w3.org/2000/svg">
      {entries.map((entry, i) => {

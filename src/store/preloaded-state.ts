...
import { KeyFrame, KeyFrameEntry, Shape } from '../lib/types';

const entityStateOf = <T extends { id: string }>(entities: T[]): EntityState<T> => ({
  ...
  keyframes: entityStateOf<KeyFrame>(
    range(randomInt(6, 9)).map(
      (i): KeyFrame => ({
        id: `keyframe${i}`,
        time: i,
        entries: range(randomInt(SHAPES.length - 3, SHAPES.length)).map((j): KeyFrameEntry => {
